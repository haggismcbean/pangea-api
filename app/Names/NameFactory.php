...
class NameFactory extends Model
{
    ...
    {
        if ($gender === "male") {
            $forenames = json_decode(file_get_contents("/www/pangea-api/app/Names/DataStores/MaleForenames.json"), true);
        } else {
            $forenames = json_decode(file_get_contents("/www/pangea-api/app/Names/DataStores/FemaleForenames.json"), true);
        }
